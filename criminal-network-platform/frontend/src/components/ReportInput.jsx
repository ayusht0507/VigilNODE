...
export default function ReportInput({ caseId, setCaseId, onIngested, onReset, persistTitle = '', setPersistTitle, persistText = '', setPersistText }) {
  ...
  // Synchronize internal inputs and displayed title when external caseId changes (e.g. from Active Cases selection)
  useEffect(() => {
    if (caseId) {
      setReportTitle(caseId)
      setLastSubmittedTitle(caseId)
    } else {
      setReportTitle('')
  ...
  useEffect(() => {
    if (persistTitle) {
      setReportTitle(persistTitle)
      setLastSubmittedTitle(persistTitle)
    } else if (!caseId) {
      setReportTitle('')
      setLastSubmittedTitle('')
    }
  ...
  useEffect(() => {
    if (persistText !== undefined) {
      setText(persistText)
      setLastSubmittedText(persistText)
    }
  }, [persistText])
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await submitTextReport(effectiveTitle, text.trim())
